...
resource "openstack_networking_port_v2" "network_ports" {
  ...
  dynamic "allowed_address_pairs" {
    for_each = var.instance.allowed_address_pair[*]
    content { # Content is what each created block from dynamic is assigned 
      ip_address = lookup(each.value, "allowed_address_pair", null)
    }
  }
}
...
resource "openstack_compute_instance_v2" "public_instance" {
    ...
    network {
        port = openstack_networking_port_v2.network_ports[count.index]
    }
